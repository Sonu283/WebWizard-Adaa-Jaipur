...
export default function SearchResults({ updateCartCount }) {
  const navigate = useNavigate();
  const [selectedProduct, setSelectedProduct] = useState(null);
  const location = useLocation();
  const { searchText } = location.state || {}; // Get search text passed as state
  ...
  // Function to fetch search results based on the search text
  const fetchSearchResults = async (searchTerm) => {
    ...
    try {
      const response = await fetch(`https://adaa-jaipur-backend.onrender.com/api/cloth/searchCloths?query=${searchTerm}`, {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          setSnackbarMessage("No products found matching your query.");
          setSnackbarOpen(true)
        ...
        setSearchResults(result); // Set the search results
        setError(''); // Clear any previous error
      }
    } catch (err) {
      console.error("Error fetching search results:", err);
      setSnackbarMessage("An error occurred while searching. Please try again later.");
      setSnackbarOpen(true)
      setError(err.message); // Set error message
    }
  };

  // Effect to fetch results when component mounts or when searchText changes
  useEffect(() => {
    if (searchText) {
      fetchSearchResults(searchText);
    }
    if (selectedProduct) {
      setSelectedColor(selectedProduct.colorD?.[0] || ""); // Set the first color as default
      setSelectedSize(selectedProduct.sizeD?.[0] || ""); // Set the first size as default
  ...
  const bannerName = "Search Result";
              ...
              <div className="bg-white rounded-lg max-w-5xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6">
                  ...
                  <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <img
                      src={selectedProduct.imageD || "/placeholder.svg"}
