...
const WishlistPage = ({ updateWishlistCount, updateCartCount }) => {
  ...
  const handleAddToCart = async (item) => {
    ...
    try {
      ...
      if (cartResponse.data?.message === "Product added to cart successfully") {
        ...
        if (wishlistResponse.data.success) {
          setWishlistItems(wishlistResponse.data.updatedWishlist);
          setWishlist(wishlistResponse.data.updatedWishlist)
          updateWishlistCount(prev => prev - 1);
          updateCartCount(prev => prev + 1);
          setSnackbarMessage("Product added to cart and removed from wishlist");
        }
      } else if (cartResponse.data.message === "Item already exists in cart") {
        setSnackbarMessage("This item is already in your cart with the same size and color.");
      } else {
        throw new Error("Unexpected response format");
      }
    } catch (err) {
      setSnackbarMessage(handleApiError(err));
    } finally {
      setSnackbarOpen(true);
  ...
  const toggleWishlist = async (product) => {
    ...
    try {
      const isInWishlist = wishlistItems.some(item => item.idD === product.idD);
      ...
      if (response.data.success) {
        ...
        if (isInWishlist) {
          ...
          setWishlistItems(prevItems => prevItems.filter(item => item.idD !== product.idD));
          setWishlist(prevItems => prevItems.filter(item => item.idD !== product.idD));
        } else {
          // Add to wishlist
  ...
  return (
    ...
    <div>
      {isLoading ? (<ProductGridSkeleton />) : error ? (
        ...
        <div>
          <div className="container space-y-4 p-3 m-0 ">
            <h1 className="text-4xl font-bold tracking-tight mb-7">
              <span className="text-[#FF6B00]">WISH</span>
              LIST
